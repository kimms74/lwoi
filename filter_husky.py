...
class HUSKYFilter():
	...
	# def extract_correct_data(self,t,)
	...
	def h_imu(self, u):
		...
		for k in range(u.shape[0]):
			...
			delta_R = delta_R_prev.mm(SO3.exp(u[k, :3].unsqueeze(dim=0).double().to(device)*self.delta_t).squeeze().float().cpu().detach())
			...
			delta_p = delta_p_prev + delta_v*self.delta_t + delta_R.mv(u[k, 3:])*(self.delta_t**2)/2
		...
		return torch.cat((SO3.log(delta_R.unsqueeze(dim=0).double().to(device)).float().cpu().detach().squeeze(),
					   delta_v,
					   delta_p), 0)
	...
	def h_hat(self, u_odo):
		...
		R0 = SO3.from_rpy(self.x_prev[3].unsqueeze(dim=0),self.x_prev[4].unsqueeze(dim=0),self.x_prev[5].unsqueeze(dim=0)).squeeze()
		# Rend = SO3.from_rpy(self.x[3:6]).as_matrix()
		Rend = SO3.from_rpy(self.x[3].unsqueeze(dim=0),self.x[4].unsqueeze(dim=0),self.x[5].unsqueeze(dim=0)).squeeze()

		p0 = self.x_prev[:3]
		p_end = self.x[:3]

		# delta_R = SO3.from_matrix(R0.t().mm(Rend)).log()
		delta_R = SO3.log(R0.t().mm(Rend).unsqueeze(dim=0).double().to(device)).float().cpu().detach().squeeze()
	...
	def compute_jac_update(self, u_odo):
		...
		Rot_prev = SO3.from_rpy(self.x_prev[3].unsqueeze(dim=0),self.x_prev[4].unsqueeze(dim=0),self.x_prev[5].unsqueeze(dim=0)).squeeze()
		# Rot_new =  SO3.from_rpy(self.x[3:6]).as_matrix()
		Rot_new =  SO3.from_rpy(self.x[3].unsqueeze(dim=0),self.x[4].unsqueeze(dim=0),self.x[5].unsqueeze(dim=0)).squeeze()
	...
	def update_cov(self, J_cor, u_odo):
		...
		ImKH = Id - K_prefix.mm(torch.linalg.solve(S, H))
		...
		self.P = ImKH.mm(self.P).mm(ImKH.transpose(-1, -2)) \
			+ K_prefix.mm(torch.linalg.solve(S, (K_prefix.mm(torch.linalg.solve(S, R))).transpose(-1, -2)))
		return K_prefix, S

	def update_state(self, K_prefix, S, dy):
		# dx = K_prefix.mm(torch.gesv(dy, S)[0]).squeeze(1) # K*dy
		dx = K_prefix.mm(torch.linalg.solve(S, dy.unsqueeze(dim=1))).squeeze(1) # K*dy
		self.x[:3] += dx[:3]
		# self.x[3:6] += (SO3.exp(dx[3:6]).dot(SO3.from_rpy(self.x[3:6]))).to_rpy()
		R1_temp = SO3.exp(dx[3:6].unsqueeze(dim=0).double().to(device)).float().cpu().detach().squeeze()
		R2_temp = SO3.from_rpy(self.x[3].unsqueeze(dim=0),self.x[4].unsqueeze(dim=0),self.x[5].unsqueeze(dim=0)).squeeze()
		self.x[3:6] += SO3.to_rpy(R1_temp.mm(R2_temp).unsqueeze(dim=0)).squeeze()
		self.x[6:9] += dx[6:9]
	...
	def compute_cate(self, t, x, chi, dataset_name):
		...
		chi_est[:, :3, :3] = SO3.from_rpy(x[:,3],x[:,4],x[:,5])
		chi_est[:, :3, 3] = x[:, :3]
		chi_est[:, 3, 3] = 1
	...
	def compute_absolute_trajectory_error(self, t, x, chi, dataset_name):
	...
	def compute_rate(self, t, x, chi, dataset_name):
		if(x.shape[0] < 200):
			delta = 60
		else:
			delta = 6000
		...
		chi_est_s = chi_est[:-delta]
		chi_est_e = chi_est[delta:]
		chi_s = chi[:-delta]
		chi_e = chi[delta:]

		chi_est_s = SE3.from_matrix(chi_est_s)
		chi_est_e = SE3.from_matrix(chi_est_e)
		chi_s = SE3.from_matrix(chi_s)
		chi_e = SE3.from_matrix(chi_e)

		chi_test = chi_est[delta:]
		error = ((chi_est_s.inv().dot(chi_est_e)).inv().dot(chi_s.inv().dot(chi_e))).log()

		rate_translation = error[:, :3].abs().mean()
		rate_rotation = error[:, 3:].abs().mean()
		return rate_translation, rate_rotation

	def compute_relative_trajectory_error(self,t,x,chi,dataset_name):
		rate_translation, rate_rotation = self.compute_rate(t, x, chi, dataset_name)
		error = {
				 'rate translation': rate_translation,
				 'rate rotation': rate_rotation,
				 }
		return error
