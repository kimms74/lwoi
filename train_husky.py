...
class GpOdoFog(Parameterized):
	...
	def plot_chi(self,u,y):
		yaw_gt = SO3.to_rpy(y[:,:3,:3])[:,2]
		yaw_u = SO3.to_rpy(u[:,:3,:3])[:,2]
		...
		ax1.plot(p_u[:,0],p_u[:,1],'orange')

		fig2, ax2 = plt.subplots(figsize=(10,10))
		ax2.plot(yaw_gt,'k')
		ax2.plot(yaw_u,'orange')
	...
	def set_data(self, u, y):
		...
		# self.gp_f.set_data(u[idx], y_diff[idx].t())
	...
	def correct(self, x, u_odo, u_fog, compute_G=False, full_cov=False):
		...
		Rot = SO3.from_rpy(x[3].unsqueeze(dim=0),x[4].unsqueeze(dim=0),x[5].unsqueeze(dim=0)).squeeze()
		# correct state
		# dRot_cor = SO3.exp(y_cor[3:]).as_matrix()
		dRot_cor = SO3.exp(y_cor[3:].unsqueeze(dim=0).double().to(device)).float().cpu().detach().squeeze()
		x[:3] = x[:3] + Rot.mv(SE3.exp(y_cor).as_matrix()[:3, 3])
		# x[3:6] = SO3.from_matrix(Rot.mm(dRot_cor)).to_rpy()
		x[3:6] = SO3.to_rpy(Rot.mm(dRot_cor).unsqueeze(dim=0)).squeeze()
		return x, G_cor
	...
	def f_hat(self, u):
		...
		# 	Rot_prev = Rot
		# 	p_prev = p
		Rot, p = self.integrate_odo_fog(u_odo,u_fog,delta_t)
	...
	def integrate_odo_fog(self, u_odo, u_fog, delta_t):
		v = u_odo.new_zeros(u_odo.shape[0],3)
		# if self.nclt:
		# 	v[:,2] = 1/2*(u_odo[:, 0] + u_odo[:, 1]).squeeze()
		# else:
		...
		# Rot = SO3.from_rpy(xi[:, 3],xi[:, 4],xi[:, 5])
		for i in range(u_odo.shape[1]):
			if self.nclt:
				v[:,0] = 1/2*(u_odo[:,i, 0] + u_odo[:,i, 1])
			else:
				v, _ = self.encoder2speed(u_odo, delta_t)
			xi[:,5] += u_fog[:,i].squeeze()
			Rot = SO3.from_rpy(xi[:,3],xi[:,4],xi[:,5])
			xi[:,:3] += bmv(Rot,v*delta_t)
		p = xi[:, :3]
		# print(p)
		return Rot, p

	def box_minus(self, chi_1, chi_2):
		return SE3.from_matrix(chi_2,True).inv().dot(SE3.from_matrix(chi_1,True)).log()
...
class GpImu(Parameterized):
	...
	def h_hat(self, u):
		...
		for k in range(u.shape[1]):	# u.shape[1]=100, 1sec
			delta_R = delta_R_prev.matmul(SO3.exp(u[:, k, :3].double().to(device)*self.delta_t).float().cpu().detach())
			...
			delta_p = delta_p_prev + delta_v*self.delta_t + bmv(delta_R, u[:, k, 3:])*(self.delta_t**2)/2
		...
		return torch.cat((SO3.log(delta_R.double().to(device)).float().cpu().detach(),
		               delta_v,
		               delta_p), 1)
def plot_chi(u,y):
	yaw_gt = SO3.to_rpy(y[:,:3,:3])[:,2]
